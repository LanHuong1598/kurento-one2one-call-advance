...
import org.kurento.tutorial.one2onecalladv.auth.Credential;
import org.kurento.tutorial.one2onecalladv.auth.UserProxy;
...
public class CallHandler extends TextWebSocketHandler {
  ...
  private void register(WebSocketSession session, JsonObject jsonMessage) throws IOException {
    ...
    if (name.isEmpty()) {
      ...
      registry.removeBySession(registry.getByName(name).getSession());
      Credential credential = new Credential();
      credential.setUsername(name);
      credential.setPassword("iTech1234");
      ResponseEntity<Object> responseResponseEntity = userProxy.login(credential);
